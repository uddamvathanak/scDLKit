...
- parameter-efficient fine-tuning support is currently `LoRA` only
...
- `CellFM`
- `Nicheformer`
- full fine-tuning baseline for the paper matrix
- adapters as a PEFT baseline
- prefix tuning
- IA3-style scaling
...
The next active milestone after installing the publication operating system is
the annotation pillar.
...
- annotation has a task spec and a benchmark matrix that distinguishes current
  truth from planned PEFT parity
- the benchmark plan covers full-label, low-label, and cross-study regimes
- the main annotation tutorial is positioned as the first paper-ready task
  tutorial
- figure-ready outputs are defined and tracked
- the milestone checklist can be executed without guessing
...
- frozen / full-FT / LoRA / adapters / prefix / IA3 comparison plan
- low-label and cross-study regime plan
